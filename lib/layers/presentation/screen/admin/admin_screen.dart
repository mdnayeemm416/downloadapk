...
class _AdminScreenBody extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<AdminBloc, AdminState>(
      ...
      listener: (context, state) {
        ...
        if (state.errorMessage.isNotEmpty && state.status == AdminStatus.loaded) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            ...
            title: Row(
              children: [
                Container(
                  ...
                  child: Icon(Icons.admin_panel_settings_rounded, size: 18, color: cs.onPrimary),
                ),
                const SizedBox(width: 10),
                Text('User Management', style: getBoldStyle(fontSize: 18, color: cs.onSurface)),
              ],
            ),
            actions: [
              // User count badge
              ...
              children: [
                ...
                Padding(
                  ...
                  child: Container(
                    ...
                    child: TextField(
                      onChanged: (v) => context.read<AdminBloc>().add(SearchUsers(v)),
                      decoration: InputDecoration(
                        hintText: 'Search by name or email...',
                        hintStyle: getRegularStyle(fontSize: 13, color: cs.onSurface.withValues(alpha: .35)),
                        prefixIcon: Icon(Icons.search_rounded, size: 20, color: cs.primary.withValues(alpha: .5)),
                ...
                // ── Content ──
                Expanded(
                  child: _buildContent(context, state, cs, isDark),
                ),
              ],
            ),
          ),
  ...
  Widget _buildContent(BuildContext context, AdminState state, ColorScheme cs, bool isDark) {
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text('Loading users...', style: getRegularStyle(fontSize: 14, color: cs.onSurface.withValues(alpha: .5))),
          ],
        ),
      );
    }
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(color: cs.error.withValues(alpha: .08), shape: BoxShape.circle),
              child: Icon(Icons.error_outline_rounded, size: 48, color: cs.error.withValues(alpha: .6)),
            ),
            const SizedBox(height: 16),
            Text('Failed to load users', style: getMediumStyle(fontSize: 16, color: cs.onSurface.withValues(alpha: .6))),
            const SizedBox(height: 8),
            Text(state.errorMessage, style: getRegularStyle(fontSize: 12, color: cs.onSurface.withValues(alpha: .4)), textAlign: TextAlign.center),
            const SizedBox(height: 20),
            TextButton.icon(
              onPressed: () => context.read<AdminBloc>().add(const LoadAllUsers()),
              icon: Icon(Icons.refresh_rounded, color: cs.primary),
              label: Text('Retry', style: getMediumStyle(fontSize: 14, color: cs.primary)),
            ),
          ],
        ),
      );
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: Icon(Icons.person_off_rounded, size: 48, color: cs.primary.withValues(alpha: .4)),
            ),
            const SizedBox(height: 16),
            Text(
              state.searchQuery.isNotEmpty ? 'No matching users' : 'No users in this category',
              style: getMediumStyle(fontSize: 16, color: cs.onSurface.withValues(alpha: .5)),
            ),
          ],
        ),
      );
        ...
        itemBuilder: (context, index) {
          ...
          return TweenAnimationBuilder<double>(
            ...
            child: _UserManagementCard(
              ...
              cs: cs,
              isDark: isDark,
              isLoading: state.actionUserId == user.id,
...
class _TabBar extends StatelessWidget {
  ...
  const _TabBar({required this.cs, required this.isDark, required this.currentTab});
        ...
        children: AdminTab.values.map((tab) {
          ...
          return Padding(
            ...
            child: AnimatedContainer(
              ...
              child: Material(
                color: isActive ? cs.primary : (isDark ? cs.onSurface.withValues(alpha: .05) : cs.primaryContainer),
                    ...
                    child: Row(
                      ...
                      children: [
                        Icon(_iconForTab(tab), size: 16,
                          color: isActive ? cs.onPrimary : cs.onSurface.withValues(alpha: .5)),
                        const SizedBox(width: 6),
                        Text(
                          _labelForTab(tab),
                          style: getMediumStyle(
                            fontSize: 13,
                            color: isActive ? cs.onPrimary : cs.onSurface.withValues(alpha: .65),
                          ),
                        ),
                      ],
                    ),
  ...
  IconData _iconForTab(AdminTab tab) {
    switch (tab) {
      case AdminTab.all: return Icons.people_rounded;
  ...
  String _labelForTab(AdminTab tab) {
    switch (tab) {
      case AdminTab.all: return 'All Users';
...
class _UserManagementCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          if (!isDark) BoxShadow(color: cs.primary.withValues(alpha: .03), blurRadius: 8, offset: const Offset(0, 3)),
        ],
      ),
      child: Column(
        children: [
              ...
              children: [
                  ...
                  children: [
                    Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(
                          color: isBlocked ? cs.error : isAdmin ? cs.secondary : cs.primary,
                          width: 2,
                        ),
                      ),
                      child: UserAvatar(username: user.username ?? '', radius: 22),
                    ),
                    if (isBlocked)
                      Positioned(
                        right: 0, bottom: 0,
                        child: Container(
                          padding: const EdgeInsets.all(2),
                          decoration: BoxDecoration(color: cs.error, shape: BoxShape.circle, border: Border.all(color: cs.surface, width: 2)),
                          child: Icon(Icons.block_rounded, size: 10, color: cs.onPrimary),
                        ),
                      ),
                    if (isAdmin && !isBlocked)
                      Positioned(
                        ...
                        child: Container(
                          padding: const EdgeInsets.all(2),
                          decoration: BoxDecoration(color: cs.secondary, shape: BoxShape.circle, border: Border.all(color: cs.surface, width: 2)),
                          child: Icon(Icons.shield_rounded, size: 10, color: cs.onSecondary),
                        ),
                      ),
                  ],
                ),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          Icon(Icons.calendar_today_rounded, size: 11, color: cs.onSurface.withValues(alpha: .3)),
                          ...
                          const SizedBox(width: 12),
                          Icon(Icons.people_outline_rounded, size: 11, color: cs.onSurface.withValues(alpha: .3)),
                          const SizedBox(width: 4),
                          Text(
                ...
                if (isLoading)
                  ...
                  PopupMenuButton<String>(
                    icon: Icon(Icons.more_vert_rounded, color: cs.onSurface.withValues(alpha: .4)),
          ...
          Padding(
            ...
            child: Row(
              children: _buildQuickActions(context, cs),
            ),
          ),
        ],
      ),
  ...
  void _handleAction(BuildContext context, String action) {
    ...
    final id = user.id ?? '';
    switch (action) {
      case 'approve': bloc.add(ApproveUser(id)); break;
      case 'reject': bloc.add(RejectUser(id)); break;
      case 'block': bloc.add(BlockUser(id)); break;
  ...
  List<PopupMenuEntry<String>> _buildMenuItems(ColorScheme cs) {
    ...
    if (isPending) {
      items.add(_menuItem('approve', Icons.check_circle_outline_rounded, 'Approve', cs.secondary));
      items.add(_menuItem('reject', Icons.cancel_outlined, 'Reject', cs.error));
      items.add(const PopupMenuDivider());
    }

    if (isBlocked) {
      items.add(_menuItem('unblock', Icons.lock_open_rounded, 'Unblock', cs.secondary));
    } else {
    ...
    if (isModerator) {
      items.add(_menuItem('remove-moderator', Icons.verified_user_outlined, 'Remove Moderator', cs.tertiary));
    } else if (!isAdmin) {
